...
def masg(call):
	...
	if call.data =="SidraTools":
		...
		while True:
			...
			usa = str(''.join(random.choice(am)for i in range(1)))
			ua = str(''.join(random.choice(xa)for i in range(1)))
			username = str(us)+str(us)+str(un)+str(usa)+str(us)
		...
		xu = "MNBVCXZLKJHGFDSAPOIUYTREWQ"
		xn = "1234567890MNBVCXZLKJHGFDSAPOIUYTREWQ"
		xa = "1234567890MNBVCXZLKJHGFDSAPOIUYTREWQ"
		xm = "_"
		ok=0
		cp=0
		sk=0
		while True:
			...
			username = str(us)+str(xm)+str(un)+str(xm)+str(ua)
			url = "https://t.me/"+str(username)
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
...
if __name__ == "__main__":
    bot.remove_webhook()
    bot.set_webhook(url="https://bruah.herokuapp.com/"+str(BOT_TOKEN))
